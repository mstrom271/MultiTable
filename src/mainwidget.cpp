...
MainWidget::MainWidget(QWidget *wgt) : QWidget(wgt) {
    ...
    connect(setupWgt, SIGNAL(start()), SLOT(startSlot()));
    stackLayout->addWidget(setupWgt);

    exerciseWgt = new ExerciseWidget();
    connect(exerciseWgt, SIGNAL(stop()), SLOT(showSetup()));
...
void MainWidget::applicationStateChanged(Qt::ApplicationState state) {
    if (stackLayout->currentWidget() == exerciseWgt) {
...
void MainWidget::showEvent(QShowEvent *event) {
    // #ifdef Q_OS_ANDROID
    //     QTimer::singleShot(500, [] { QtAndroid::hideSplashScreen(200); });
    // #endif
